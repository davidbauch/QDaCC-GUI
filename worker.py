...
class QDaCCOptimizer(QDaCCThread):
    ...
    def readFiles(self, path: str, files: list[str], indices: list[list[int]]) -> list[tuple]:
        ...
        for file, index in zip(files, indices):
            with open(join(path,file),"r") as f:
                ...
                content_transposed = list(zip(*[[float(a) for a in line.split()] for line in content]))
    ...
    def evaluate(self, params: list[float]):
        ...
        # Update Variables
        self.eval_dict.update({f"V{i}" : eval(p, self.eval_dict) if p != "" else "" for i,(k,p) in enumerate(self.variables.items(),1)})
        ...
        self.eval_dict.update({f"Y{i}" : p[1] for i,p in enumerate(data,1)}) 
        ...
        self.outset.emit(f"Current parameters are {self.current_parameters}, current Fitness is {quality}")
    ...
    def callback(self, current_parameters):
        ...
        self.outset.emit(f"Current parameters are {current_parameters}, current Fitness is {self.quality_data[-1]}")
    ...
    def run(self):
        if "command" not in self.kwargs:
            raise Exception("No command specified for QDaCCOptimizer")
        runstring = self.kwargs["command"]
        if not "{" in runstring or not "}" in runstring:
            raise Exception("No parameter in runstring for QDaCCOptimizer")
